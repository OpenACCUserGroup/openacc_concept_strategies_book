...
double norm_temp1[2]:[*], norm_temp2[2]:[*], red_temp:[*];
...
#pragma acc data pcopyin(colidx[0:nz], rowstr[0:size_rowstr], x[0:size_x], z[0:size_x], a[0:nz], p[0:size_x], q[0:size_x], r[0:size_x], w[0:size_x]) pcopyin(reduce_recv_lengths[0:num_proc_cols])
   {
/*--------------------------------------------------------------------
c---->
...
#pragma xmp reduction(+:norm_temp1) on subproc
     ...
     for( i = 1; i <= l2npcols; i++){
       if (timeron) timer_start(t_ncomm);
       xmp_sync_image(reduce_exch_proc[i-1]+1, NULL);
       norm_temp2[:]:[reduce_exch_proc[i-1]+1] = norm_temp1[:];
       xmp_sync_image(reduce_exch_proc[i-1]+1, NULL);
       if (timeron) timer_stop(t_ncomm);
...
c-------------------------------------------------------------------*/
  ...
  for( i = 1; i <= *l2npcols; i++){
    ...
    reduce_exch_proc[i-1] = proc_row*npcols + j;
  ...
  for( i = *l2npcols; i >= 1; i--){
    ...
    if( nprows == npcols ){
      reduce_send_starts[i-1]  = send_start;
      reduce_send_lengths[i-1] = send_len;
      reduce_recv_lengths[i-1] = lastrow - firstrow + 1;
    } else {
      reduce_recv_lengths[i-1] = send_len;
      if( i == *l2npcols ){
	reduce_send_lengths[i-1] = lastrow-firstrow+1 - send_len;
	if( me/2*2 == me ){
	  reduce_send_starts[i-1] = send_start + send_len;
	} else {
	  reduce_send_starts[i-1] = 1;
	}
      } else {
	reduce_send_lengths[i-1] = send_len;
	reduce_send_starts[i-1]  = send_start;
    ...
    reduce_recv_starts[i-1] = send_start;
...
c-------------------------------------------------------------------*/
  ...
  int size_x = na/num_proc_rows+2; //for acc
  int size_rowstr = na+1; //for acc
#pragma acc data pcopy(colidx[0:nz], rowstr[0:size_rowstr], x[0:size_x], z[0:size_x], a[0:nz], p[0:size_x], q[0:size_x], r[0:size_x], w[0:size_x]) pcopy(reduce_recv_lengths[0:l2npcols])
...
#pragma xmp reduction(+:sum) on subproc
  ...
  for( i = 0; i < l2npcols; i++){
    if (timeron) timer_start(t_rcomm);
    xmp_sync_image(reduce_exch_proc[i]+1, NULL);
    red_temp:[reduce_exch_proc[i]+1] = sum;
    xmp_sync_image(reduce_exch_proc[i]+1, NULL);
    if (timeron) timer_stop(t_rcomm);
...
#pragma xmp reduction(+:w) on subproc(:) acc
    ...
    for(i = l2npcols; i >= 1; i--){
      if (timeron) timer_start(t_rcomm);
      int reduce_proc = reduce_exch_proc[i-1] + 1; //1-based
      xmp_sync_image(reduce_proc, NULL);
#ifdef USE_GET
      int reduce_start = reduce_recv_starts[i-1]-1;
      int reduce_length = reduce_recv_lengths[i-1];
      q[reduce_start : reduce_length]
	= w[reduce_start : reduce_length]:[reduce_proc];
#else
      int reduce_start = reduce_send_starts[i-1]-1;
      int reduce_length = reduce_send_lengths[i-1];
      q[reduce_start : reduce_length]:[reduce_proc]
	= w[reduce_start : reduce_length];
#endif
      xmp_sync_image(reduce_proc, NULL);
      if (timeron) timer_stop(t_rcomm);
#pragma acc parallel loop pcopy(w[0:size_x], q[0:size_x], reduce_recv_lengths[0:l2npcols])
      for( j=send_start -1; j < send_start + reduce_recv_lengths[i-1] - 1; j++){
	w[j] = w[j] + q[j];
      }
    }
#endif
...
#else
      q[0 : send_len]:[exch_proc+1] = w[send_start-1 : send_len];
#endif
      xmp_sync_image(exch_proc+1, NULL);

      if (timeron) timer_stop(t_rcomm);
    }else{
#pragma acc parallel loop pcopy(q[0:size_x], w[0:size_x])
...
#pragma xmp reduction(+:sum) on subproc
    ...
    for(i = 1; i <= l2npcols; i++){
      if (timeron) timer_start(t_rcomm);
      xmp_sync_image(reduce_exch_proc[i-1]+1, NULL);
      red_temp:[reduce_exch_proc[i-1]+1] = sum;
      xmp_sync_image(reduce_exch_proc[i-1]+1, NULL);
      if (timeron) timer_stop(t_rcomm);
...
#pragma xmp reduction(+:w) on subproc(:) acc
  ...
  for(i = l2npcols; i >= 1; i--){
    ...
    int reduce_start = reduce_recv_starts[i-1]-1;
    int reduce_length = reduce_recv_lengths[i-1];
    r[reduce_start : reduce_length]
      = w[reduce_start : reduce_length]:[reduce_proc];
#else
    int reduce_start = reduce_send_starts[i-1]-1;
    int reduce_length = reduce_send_lengths[i-1];
    r[reduce_start : reduce_length]:[reduce_proc]
      = w[reduce_start : reduce_length];
#endif
    xmp_sync_image(reduce_proc, NULL);
     if (timeron) timer_stop(t_rcomm);

#pragma acc parallel loop pcopy(w[0:size_x], r[0:size_x], reduce_recv_lengths[0:l2npcols])
     for(j=send_start-1; j < send_start + reduce_recv_lengths[i-1] - 1; j++){
       w[j] = w[j] + r[j];
     }
  }
...
c-------------------------------------------------------------------*/
  ...
  if( l2npcols != 0 ){
    ...
    xmp_sync_image(exch_proc+1, NULL);
#ifdef USE_GET
    r[0 : send_len] = w[send_start-1 : send_len]:[exch_proc+1];
#else
    r[0 : send_len]:[exch_proc+1] = w[send_start-1 : send_len];
#endif
    xmp_sync_image(exch_proc+1, NULL);
    if (timeron) timer_stop(t_rcomm);
  }else{
#pragma acc parallel loop pcopy(r[0:size_x], w[0:size_x])
    for(j=0; j < exch_recv_length; j++){
      r[j] = w[j];
    }
  }
